...
var QueryGenerator = {
  ...
  changeColumnQuery: function(tableName, attributes) {
    ...
    for (var attributeName in attributes) {
      ...
      if (attributes[attributeName].match(/^ENUM\(/)) {
        ...
        sql.push(`ALTER TABLE ${this.quoteTable(tableName)} ALTER COLUMN ${this.quoteIdentifier(attributeName)} DROP DEFAULT;`);
        sql.push(this.pgEnumDrop(tableName, attributeName + "_old") + this.pgEnumRename(tableName, attributeName)
          + this.pgEnum(tableName, attributeName, attributes[attributeName]));
        query = query
          + this.pgEnumDrop(tableName, attributeName + "_old");
        definition = definition.replace(/^ENUM\(.+\)/, this.quoteIdentifier('enum_' + tableName + '_' + attributeName));
        //definition has already quoteIdentifiered.
        definition += ' USING ((' + this.quoteIdentifier(attributeName) + '::text)::' + definition + ')';
      }
  ...
  pgEnumRename: function(tableName, attr, enumNewName) {
    var enumName = this.pgEscapeAndQuote('enum_' + tableName + '_' + attr);
    enumNewName = enumNewName || this.pgEscapeAndQuote('enum_' + tableName + '_' + attr + '_old');
    var enumNameStr = enumName.substr(1, enumName.length - 2);
    return `DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_type WHERE typname = '${enumNameStr}') THEN 
    ALTER TYPE ${enumName} RENAME TO ${enumNewName};
    END IF; END $$;
`;
  },
